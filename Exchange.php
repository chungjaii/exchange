...
class Exchange {
     ...
     function redis(){
        ...
        $redis = new Predis\Client(array(
            "scheme" => "tcp",
            "host" => "redis-10104.c15.us-east-1-2.ec2.cloud.redislabs.com",
            "port" => "10104"
            ));
     ...
     public function getCacheResult(){
        ...
        $radis = $this->redis();
        $cacheResult = $radis->get($this->currencyFrom);
        if(isset($cacheResult)){
            $conversionResult = json_decode($cacheResult, true);
            $radis->set($this->currencyFrom,$json);
            $exchangeRate = $conversionResult['rates'][$this->currencyTo];
            $amount = $this->quantity * $exchangeRate;
            $result = array("status"=>"400","rates"=>$exchangeRate, 'amount'=>$amount, 'cache' => 'true');
            ...
            $exchangeRate = $conversionResult['rates'][$this->currencyTo];
            $amount = $this->quantity * $exchangeRate;
            $result = array("status"=>"400","rates"=>$exchangeRate, 'amount'=>$amount , 'cache' => 'false');
            return json_encode($result);
